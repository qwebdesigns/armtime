    ...
    fetch(filePath)
        ...
        .then((data) => {
            ...
            for (let i = 0; i < categoryData.length; i++) {
                if (
                    weight >= parseInt(categoryData[i]["Minimum Participant Weight"]) &&
                    weight <= parseInt(categoryData[i]["Maximum Participant Weight"])
                ) {
...
function getAllParsentages(currentCat, currentDat) {
    ...
    percentagesAll = {
        "Строгий подъем на бицепс": Math.floor(
            calculatePercentage(
                min_max_values["MIN Biceps"],
                min_max_values["MAX Biceps"],
                currentDat["Строгий подъем на бицепс"]
            )
        ),
        "Сгибатель кисти": Math.floor(
            calculatePercentage(
                min_max_values["MIN Wrist Flexor"],
                min_max_values["MAX Wrist Flexor"],
                currentDat["Сгибатель кисти"]
            )
        ),
        "Боковое давление": Math.floor(
            calculatePercentage(
                min_max_values["MIN Side Pressure"],
                min_max_values["MAX Side Pressure"],
                currentDat["Боковое давление"]
            )
        ),
        "Подъем на луч": Math.floor(
            calculatePercentage(
                min_max_values["MIN Lift on Radius"],
                min_max_values["MAX Lift on Radius"],
                currentDat["Подъем на луч"]
            )
        ),
        Пронация: Math.floor(
            calculatePercentage(
                min_max_values["MIN Pronation"],
                min_max_values["MAX Pronation"],
                currentDat["Пронация"]
            )
        ),
        Отведение: Math.floor(
            calculatePercentage(
                min_max_values["MIN Abduction"],
                min_max_values["MAX Abduction"],
                currentDat["Отведение"]
            )
        ),
        "Французский жим": Math.floor(
            calculatePercentage(
                min_max_values["MIN French Press"],
                min_max_values["MAX French Press"],
                currentDat["Французский жим"]
            )
        ),
    };
    ...
    let ringPercentages = {
        ring3: Math.floor(
            (percentagesAll["Французский жим"] + percentagesAll["Боковое давление"]) /
            2
        ),
        ring2: Math.floor(
            (percentagesAll["Подъем на луч"] +
                percentagesAll["Пронация"] +
                percentagesAll["Отведение"]) /
            3
        ),
        ring1: Math.floor(
            (percentagesAll["Строгий подъем на бицепс"] +
                percentagesAll["Сгибатель кисти"] +
                percentagesAll["Боковое давление"]) /
            3
        ),
    };

    let ring0 = Math.floor(
        (ringPercentages["ring1"] +
            ringPercentages["ring2"] +
            ringPercentages["ring3"]) /
        3
    );
    ...
    document.getElementById("member_weight").textContent =
        currentDat["Вес участника"] + " кг";

    document.getElementById("circle0").style = "--percentage: " + ring0 + "%;";
    document.getElementById("ring0_ball").innerHTML = ring0;

    document.getElementById("circle1").firstElementChild.textContent =
        ringPercentages["ring1"];
    document.getElementById("circle1").style =
        "--percentage: " + ringPercentages["ring1"] + "%;";

    document.getElementById("circle2").firstElementChild.textContent =
        ringPercentages["ring2"];
    document.getElementById("circle2").style =
        "--percentage: " + ringPercentages["ring2"] + "%;";

    document.getElementById("circle3").firstElementChild.textContent =
        ringPercentages["ring3"];
    document.getElementById("circle3").style =
        "--percentage: " + ringPercentages["ring3"] + "%;";
    ...
    if (
        currentDat["Карточка"] &&
        currentDat["Карточка"] != card_variant[0]["Название карточки"]
    ) {
